...
# $env:NODE_PATH = "C:\Users\yaozh\AppData\Roaming\npm"
...
# . 'C:\Users\yaozh\Documents\WindowsPowerShell\Modules\posh-git\profile.example.ps1'
...
# . 'C:\Users\yaozh\powershell\Get-NetworkStatistics.ps1'
...
function prompt {
    ...
    $prompt = ""
		...
		$prompt += Write-Prompt "$env:username"
		$prompt += Write-Prompt "`@" -ForegroundColor DarkGreen
		$prompt += Write-Prompt "$env:computername" -ForegroundColor DarkYellow
		$prompt += Write-Prompt "`:" -ForegroundColor Red
    $prompt += Write-Prompt "$(Truncate-Directory("$pwd"))" -ForegroundColor Cyan
    $prompt += Write-VcsStatus
    $prompt += Write-Prompt "$(if ($PsDebugContext) {' [DBG]: '} else {''})" -ForegroundColor Magenta
    $prompt += Write-Prompt "$(' ' * ($nestedPromptLevel + 1))`n$" -ForegroundColor Red

    $LASTEXITCODE = $origLastExitCode
    if ($prompt) {$prompt} else {" "}
}

function Truncate-Directory($Path) {
  $Path.Replace("$home", "~").Split("\")[-1]
}
...
function load-python {
	& 'C:\Users\yaozh\AppData\Local\Miniconda3\Scripts\activate.ps1'
}
